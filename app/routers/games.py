...
def create_game(game: GameCreate, session) -> Game:
    return games.create_game(session, game)


@router.get("/games", response_model=list[Game]) 
def get_games(session) -> list[Game]:
...
def update_game(game_id: int, params: GameUpdate, session) -> Game:
    return games.update_game(session, game_id, params)


@router.delete("/games/{game_id}", response_model=Game) 
def delete_game(game_id: int, session) -> Game:
